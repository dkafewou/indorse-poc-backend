...
describe('Claims', () => {
  ...
  before('updating claim, create a new one', (done) => {
    ...
    /* added hash resending test*/
    describe('POST /resendHash', () => {
        it('should return 200 and mail hash to user', (done) => {
            chai.request(server)
                .post('/resendHash/' + update_claim_id)
                .set('Authorization', 'Bearer ' + token)
                .end((err, res) => {
                    res.should.have.status(200)
        ...
        it('should return 404 if claim not found', (done) => {
            chai.request(server)
                .post('/resendHash/59a6ad282e7e26a8b402junk')
                .set('Authorization', 'Bearer ' + token)
                .end((err, res) => {
                    res.body.should.be.a('object')
  ...
  describe('POST /updateclaim', () => {
    ...
    /* added repo verification test*/
    describe('POST //verify-repo-ownership', () => {
        it('should return 200 and confirm the verification', (done) => {
            chai.request(server)
                .post('/verify-repo-ownership/' + update_claim_id)
                .set('Authorization', 'Bearer ' + token)
                .end((err, res) => {
                    res.should.have.status(200)
                    res.body.message.should.equal('Repo verified')
                    done()
                })
        })
        ...
        it('should return 404 if claim not found', (done) => {
            chai.request(server)
                .post('/verify-repo-ownership/59a6ad282e7e26a8b402junk')
                .set('Authorization', 'Bearer ' + token)
                .end((err, res) => {
                    res.body.should.be.a('object')
                    res.should.have.status(404)
