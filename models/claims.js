                ...
                collection.findOne({
                    ...
                    if (item) {
                        ...
                        if ('claim_id' in info && info['claim_id'] != '') {
                            ...
                            claim['state'] = 'new';
                            claim['visible'] = true
                            claim['repoVerified'] = false;/* adding attribute repoVerified to claim*/
                            claim['hash'] = randtoken.generate(16);/* adding unique hash related to every claim*/
                                ...
                                collection1.insert(claim, {
                                    ...
                                    if (err) {
                                        ...
                                        if ('result' in result && 'ok' in result['result'] && result['result']['ok'] == 1) {
                                            /* mailing the hash to the user if claim successfully created*/
                                            var msg_text = "Dear " + item['name'] + ", <br><br> Thank you for making a claim on Indorse.io.  Use this hash below to confirm the repository ownership.<br><br> Hash: "+ claim['hash'] +"<br><br> Add the hash above to your repository and come to the app and make a confirmation request.<br><br> Thank you and regards <br> Team Indorse <br><br> Please let us know if you have any problems or questions at: <br> www.indorse.io";
                                            var sub_text = 'Your repo ownership verification hash from Indorse';
                                            var data = {
...
exports.resendHash = function (req, res) {
    if('login' in req.body && req.body.login) {
        var claim_id = req.params.claim_id;
        var info = req.body;
        db.collection('claims', function (err, collection) {
            collection.findOne({'_id': new ObjectID(claim_id)}, function (err, item) {
                if (item) {
                    if(!item['repoVerified']){
                        /* resending  the hash to the user if request*/
                        var msg_text = " Thank you for making a claim on Indorse.io.  Use this hash below to confirm the repository ownership.<br><br> Hash: "+ item['hash'] +"<br><br> Add the hash above to your repository and come to the app and make a confirmation request.<br><br> Thank you and regards <br> Team Indorse <br><br> Please let us know if you have any problems or questions at: <br> www.indorse.io";
                        var sub_text = 'Your repo ownership verification hash from Indorse';
                        var data = {
                            from: 'Indorse <info@indorse.io>',
                            to: info['email'],
                        ...
                        mailgun.messages().send(data, function (error, response) {
                            res.send(200, {success: true, message: config.get('Msg3')})
                        });
                    }
                } else {
...
exports.repoVerify = function (req, res) {
    if('login' in req.body && req.body.login) {
        var claim_id = req.params.claim_id;
        db.collection('claims', function (err, collection) {
            ...
            collection.findOne({'_id': new ObjectID(claim_id)}, function (err, item) {
                if (item) {
                    var url = item['proof'].toString()+"/blob/master/"+item['hash'];
                    request.get(url, function (err, response, body) {
                        if(!err && response.statusCode===200){
                            item['repoVerified'] = true;
                            collection.update({'_id': new ObjectID(claim_id)}, item, {safe: true}, function(err, result) {
                                if (err) {
                                    res.send(501, {success: false, message: config.get('Msg37')});
                                } else {
                                    create_votinground(claim_id, item['ownerid']);
                                    res.send(200, {success: true,claim: result['ops'], message: "Repo verified"});
                                }
                            })
                        } else {
                            res.send(405,{ success : false, message : "Hash not found. Click on resend the hash if you don't get the original email" });
                        }
                    })
